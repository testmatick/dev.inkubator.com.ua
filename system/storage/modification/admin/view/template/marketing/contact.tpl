...
CKEDITOR.replace('input-message', {
	...
	on: {
		instanceReady: function(evt) {
			$('#cke_input-message').after('<input type="hidden" id="filemanager" value="" />');
			$('#cke_input-message .cke_button__image').before('<a href="javascript:;" class="filemanager cke_button" title="<?php echo $text_filemanager; ?>"><span class="cke_button_icon">&nbsp;</span></a>');
			$(document).on('click', '#cke_input-message .filemanager', function() {
				$('#modal-image').remove();
				$.ajax({
					url: 'index.php?route=common/filemanager&token=<?php echo $token; ?>&target=filemanager',
					dataType: 'html',
					success: function(html) {
						$('body').append('<div id="modal-image" class="modal filemanager-message">' + html + '</div>');
						$('#modal-image').modal('show');
					}
				});
			});
			$('body').on('click', '.filemanager-message .thumbnail', function (){
